...
The app was seeded with intentional defects in **two tiers** (see
[`context/bugs/001/bug-context.md`](context/bugs/001/bug-context.md)). Tier 1 was
remediated in an earlier hardening pass (kept for traceability and re-confirmed by
the Security Verifier); Tier 2 is what **pipeline run 001** actually discovered,
fixed, and regression-tested.

**Tier 2 — found & fixed in run 001**

| ID | Type | Severity | Description |
|----|------|----------|-------------|
| BUG-001 | Functional | MEDIUM | `add_expense()` accepted `NaN`/`Infinity` via `float()` — one poisoned record made every `get_total()` return `nan`/`inf` |
| BUG-002 | Functional | LOW | Blank/whitespace category accepted; empty-string filter (`if category:`) silently returned **all** expenses |
| BUG-003 | Functional | LOW | `get_expenses()` returned live dict references — a caller could mutate the internal store |
| SEC-001 | Security (INFO) | INFO | Raw `amount` echoed in `ValueError` (CWE-209); no change required in a CLI context |

**Tier 1 — previously remediated (confirmed resolved by Security Verifier)**

| ID | Type | Severity | Description |
|----|------|----------|-------------|
| BUG-001* | Functional | HIGH | `get_total()` used `EXPENSES[1:]` — first expense always skipped |
| BUG-002* | Functional | MEDIUM | `get_expenses(category)` used `==` — case-sensitive category filter |
| SEC-001* | Security | CRITICAL | `add_expense()` used `eval(amount)` — arbitrary code execution (RCE) |

### After pipeline

All run-001 findings resolved and the earlier CRITICAL `eval()` RCE confirmed
closed. Full test suite: **31 passed, 0 failed** (18 core + 13 generated).
...
| `test-report.md` | Unit Test Generator | 13 generated FIRST-compliant tests; all pass |
...
│   └── test_generated.py            ← 13 FIRST-compliant tests (generated by Unit Test Generator)
├── context/bugs/001/
